...
float map( vec2 c ) 
{
	return 20.0*texture2D( iChannel0, fract((c+0.5)/gl_FragCoord.xy), -100.0 ).x;
}
...
vec4 CapsuleColor( in vec3 pos, in vec3 nor, in float hei, in float cid )
{
    ...
    vec3 te = texcube( iChannel3, 0.7*pos+ 13.13*cid, nor ).xyz;
...
void main( void )
{
	...
	for( int a=0; a<VIS_SAMPLES; a++ )
	{
        ...
        if( t>0.0 )
        {
			...
			col += DirLight( lig, 5.0*vec3(2.8,1.5,1.0), nor, -rd, mate.xyz, mate.w, sha );
			
            // back light			
			vec3  blig = normalize(vec3(-lig.x,0.0,-lig.z));
            col += DirLight( blig, vec3(0.9,0.8,0.7), nor, -rd, mate.xyz, 1.0, occ );
