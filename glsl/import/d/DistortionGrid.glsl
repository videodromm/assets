...
const vec3 colorTwo = vec3(0.7, 0.3, 0.0);
const vec3 colorThree = vec3(0.0, 0.0, 0.0);
...
vec3 normal(vec2 uv, vec2 mp, float a) {
    vec2 offsetX = vec2(  a, 0.0) / iResolution.xy;
    vec2 offsetY = vec2(0.0, 1.0) / iResolution.xy;
...
vec3 getCol(vec2 uv, vec2 mp, float a) {
    ...
    vec3 c = (b > 0.0 ? mix(iBackgroundColor, iColor, step(0.5, b)) : colorThree);
...
vec3 edge(vec2 uv, vec2 mp, float a) {
    ...
    for (int i=0; i < 3; ++i)
    {
        for (int j=0; j < 3; ++j)
        {
            ...
            vec2 p = (gl_FragCoord.xy + os) / iResolution.xy * 2.0 - 1.0;
            p.x *= a;
