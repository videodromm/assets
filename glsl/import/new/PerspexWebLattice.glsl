...
/*
    ...
    not exactly sure what it's supposed to be... My best guess it that it's a monitor system 
    designed by an Alien race with no common sense who have access to physics defying 
    materials? :)
...
vec3 tex3D( sampler2D tex, in vec3 p, in vec3 n ){
    ...
    n = max((abs(n) - .2)*72., .001); // Roughly normalized. Ryan Geiss is a clever guy.
    n /= (n.x + n.y + n.z );  
...
void main(void) {
    ...
    if(t<FAR){
        ...
        c.xyz = c.xyz*(df + .75) + vec3(1, .97, .92)*sp;
    ...
    gl_FragColor = vec4(sqrt(clamp(c.xyz, 0., 1.)), 1.);
}
