...
{
	float j = sin(uvTunnel.y*1.0*3.14+uvTunnel.x*0.0+iGlobalTime*5.0);
	float i = sin(uvTunnel.x*10.0-uvTunnel.y*2.0*3.14+iGlobalTime*10.0);
 ...
 void main(void)
{
   ...
   uv = -0.8 + iZoom * uv * 3.0 ;
	...
	uv += vec2(sin(iGlobalTime*0.4), sin(-iGlobalTime*0.2));


	float r = sqrt(dot(uv,uv));
	float a = atan(uv.y*(0.3+0.2*cos(iGlobalTime*2.0+uv.y)),uv.x*(0.3+0.2*sin(iGlobalTime+uv.x)))+iGlobalTime;
	
   	vec2 uvTunnel;
    uvTunnel.x = iGlobalTime + 1.0/( r + .01);
