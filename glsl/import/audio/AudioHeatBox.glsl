...
// https://www.shadertoy.com/view/XdSGWR
// **************************************************************************
// CONFIG OPTIONS
                ...
                vec3 rdir )
{
    ...
    {
        float distatten = max(0., 1. - reflmarch.x/20.);
        distatten *= distatten;
...
void animateGlobals()
{
    ...
    for (int i = 0; i < NUM_AUDIO_SAMPLES; i += 1) {
        ...
        g_audioFreqs[i] = smoothstep(0.2, 0.8, 
                                 texture2D( iChannel0, 
                                 vec2(0.05 + offsets, 0.01)).b);
    }
