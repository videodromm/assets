...
/*
	...
	with the exact starting position (on the surface of the volumetric ray).
...
//RANGE 0...1
#define RAY_THICKNESS 0.62
#define RAY_BRIGHTNESS .85
...
//(i tried using a proper helix sdf, but it wasnt as deformable)
...
vec2 map(vec3 p)
{
    ...
    //some noise for good measure.
    rz.x *= 1.1-noise(p.x*1.+p.z*10.+time*16.)*.12;
...
{	
    ...
    vec3 ro2 = vec3(sin(-time*1.5)*0.3,cos(-time*1.4)*0.3,-17.5)+path(ptx);
	vec3 ta2 = vec3(0,0.,-14.)+path(ptx);
    vec3 ro3 = vec3(5.,2.,sin(time*.64)*10.-9.)+path(ptx);
	vec3 ta3 = vec3(0,0.,sin(time*.64)*3.-9.)+path(ptx);
