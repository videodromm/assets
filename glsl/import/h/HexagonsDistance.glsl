...
float HDhash1( vec2  p ) { float n = dot(p,vec2(127.1,311.7) ); return fract(sin(n)*43758.5453); }

float HDnoise( in vec3 x )
{
    vec3 p = floor(x);
    vec3 f = fract(x);
	f = f*f*(3.0-2.0*f);
...
void main(void)
{
   vec2 uv = iZoom * (gl_FragCoord.xy/iResolution.xy + 1.0);
   uv.x -= iRenderXY.x;
   uv.y -= iRenderXY.y;
