...
// hacer que las bolas se esquiven, alguna animacion interesnta
// optimizar el antialias
...
void main( void )
{
	...
	for( int i=0; i<NUMSPHEREES; i++ )
	{
	    ...
	    vec3  pos = 1.0*cos( 6.2831*hash3(id*137.17) + 1.5*(1.0-0.7*ra)*hash3(id*431.3+4.7)*time*2.0 );
        //ra = 0.3 + 0.7*ra;
        ra = 0.2 + 0.8*ra;
    ...
    for( int j=0; j<NUMSPHEREES; j++ )
    {
        if( j!=i )
        {
            ...
            float ra = sphere[i].w + sphere[j].w;
            if( dot(di,di) < ra*ra )
            {
                float l = length(di);
                vec3 o  = 0.5*di*(l-ra)/l;
                sphere[i].xyz -= o;
                sphere[j].xyz += o;
            }
        }
