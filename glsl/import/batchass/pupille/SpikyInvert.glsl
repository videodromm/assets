...
void CamPolar( out vec3 pos, out vec3 ray, in vec3 origin, in vec2 rotation, in float distance, in float zoom )
{
	...
	//ray.xy = gl_FragCoord.xy/iResolution.xy - gl_FragCoord.xy/iResolution.xy*.5;
	//ray.z = gl_TexCoord[0].t*zoom;
	ray = normalize(ray);
//	localRay = ray;
...
void main(void)
{
	vec2 camRot = vec2(0.4,2.6)+vec2(-2.5,8.5)*(iMouse.yx/gl_FragCoord.xy/iResolution.xy) ;
